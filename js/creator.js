console.log("🚀 ~ cardPicPaths:", cardPicPaths);
...
for (let k = 0; k < ctypeArray.length; k++) {
  ctype.innerHTML += `<option value="${ctypeArray[k]}">${
    cardDATA[ctypeArray[k]]["title"]
  } card</option>`;
}
...
function selectCard(no) {
  ...
  //   `/templates/1.html?card=${ctype}&name=${enc_cname}&text=${enc_ctext}&templ=${templ}`; // for local
  temp =
    window.location.href +
    `templates/1.html?card=${ctype}&name=${enc_cname}&text=${enc_ctext}&templ=${templ}`; // for github
  res.value = temp;
  ...
  document.getElementById(
    "msg"
  ).innerText = `Card ${templ} is Selected. Send the link or the QR Code to the person`;
}
